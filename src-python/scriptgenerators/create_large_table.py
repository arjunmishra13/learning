...
def create_large_table(db_name, table_name, number_of_columns, number_of_partitions, outputfile):
  ...
  create_table_command1 = "CREATE TABLE " + db_name + DOT + table_name + SPACE + CURLY_OPEN_BRACKETS
  ...
  for part_val in range(1, number_of_partitions + 1):
    partition_command = base_partition_command + COL + str(number_of_columns) + SPACE + EQUALS + SPACE + str(part_val) + CURLY_CLOSE_BRACKETS
    partition_command = partition_command + SPACE + location_str + db_name + DOT_DB + FORWARD_SLASH + table_name + FORWARD_SLASH + SINGLE_QUOTES + SEMICOLON + END_CHARACTER
